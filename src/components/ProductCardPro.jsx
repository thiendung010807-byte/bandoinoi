      ...
      >
        ...
        <div className="aspect-square rounded-xl md:rounded-2xl bg-slate-50 overflow-hidden relative mb-3 md:mb-4 cursor-pointer" onClick={() => setIsModalOpen(true)}>
          ...
          {isOutOfStock ? (
            ...
            <div className="absolute inset-0 bg-slate-900/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-10"><button className="bg-white text-slate-900 rounded-full px-4 py-2 md:px-6 md:py-2.5 font-bold text-xs md:text-sm shadow-xl transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 hover:bg-green-500 hover:text-white">Chi tiết</button></div>
          )}
        </div>
        ...
        <div className="flex flex-col flex-grow cursor-pointer" onClick={() => setIsModalOpen(true)}>
          ...
          <div className="mt-2 mb-3 md:mt-3 md:mb-4">
            ...
            <div className="w-full bg-slate-100 rounded-full h-1 md:h-1.5 overflow-hidden">
              <motion.div initial={{ width: 0 }} whileInView={{ width: `${progressPercent}%` }} viewport={{ once: true }} transition={{ duration: 0.6, ease: 'easeOut' }} className={`h-1 md:h-1.5 rounded-full ${isHot ? 'bg-gradient-to-r from-orange-400 to-orange-500' : 'bg-slate-300'}`} />
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center md:p-6 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsModalOpen(false)} className="absolute inset-0 z-0" />
            <motion.div initial={{ opacity: 0, y: "100%" }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-white rounded-t-[2rem] md:rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl relative flex flex-col md:flex-row z-10">
              <button onClick={() => setIsModalOpen(false)} className="absolute top-3 right-3 md:top-4 md:right-4 z-20 bg-white/80 backdrop-blur-md p-2 rounded-full text-slate-500 hover:text-red-500 hover:bg-red-50 transition-colors shadow-sm"><X className="w-5 h-5 md:w-6 md:h-6" /></button>
              ...
              <div className="md:w-5/12 bg-slate-50 p-4 md:p-6 flex flex-col items-center border-b md:border-b-0 md:border-r border-slate-100">
                <div className="w-full aspect-video md:aspect-square rounded-2xl overflow-hidden bg-white shadow-sm border border-slate-200 mb-3 md:mb-4 relative">
                  <AnimatePresence mode="wait"><motion.img key={currentImage} initial={{ opacity: 0.5 }} animate={{ opacity: 1 }} exit={{ opacity: 0.5 }} transition={{ duration: 0.2 }} src={currentImage} alt={product.name} className="absolute inset-0 w-full h-full object-cover" /></AnimatePresence>
                </div>
                {allImages.length > 1 && (
